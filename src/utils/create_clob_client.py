...
class ClobClient:
    ...
    async def create_market_order(self, order_args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Prefer create_order with explicit price/size since this bot already fetches top-of-book
        # and sets a limit price for FOK execution.
        if hasattr(self._sdk_client, 'create_order'):
            try:
                ...
                error(f'create_order path failed, trying create_market_order fallback: {e}')

        # Fallback to SDK create_market_order for variants that do not expose create_order.
        if hasattr(self._sdk_client, 'create_market_order'):
            ...
            try:
                ...
                # SDK market order amount is denominated in quote units for BUY and base units for SELL.
                market_amount = amount if side == 'BUY' else (amount / price if price > 0 else amount)
        ...
        raise RuntimeError('SDK client has neither create_order nor create_market_order methods')
